...
///
/// Die Zielwerte gelten fuer ein ganzes Workout, nicht fuer einen Satz —
/// 50 Klimmzuege oder 100 Liegestuetze verteilt auf beliebig viele
/// Durchgaenge. Fruehere Werte von 10 bis 15 waren in wenigen Minuten
/// erledigt und liessen den Fortschrittsbalken sofort volllaufen.
const allQuickChipExercises = [
  QuickChipExercise(
      name: 'Pull Ups', icon: Icons.fitness_center_rounded, defaultReps: 50),
  QuickChipExercise(
      name: 'Chin Ups', icon: Icons.fitness_center_rounded, defaultReps: 50),
  QuickChipExercise(
      name: 'Dips', icon: Icons.fitness_center_rounded, defaultReps: 50),
  QuickChipExercise(
      name: 'Push Ups', icon: Icons.sports_gymnastics_rounded, defaultReps: 100),
  QuickChipExercise(name: 'Rows', icon: Icons.rowing_rounded, defaultReps: 50),
  QuickChipExercise(
      name: 'Squats', icon: Icons.directions_walk_rounded, defaultReps: 100),
  QuickChipExercise(
      name: 'Lunges', icon: Icons.directions_run_rounded, defaultReps: 50),
  QuickChipExercise(
      name: 'Leg Raises',
  ...
  QuickChipExercise(
      ...
      defaultReps: 50),
  QuickChipExercise(
      name: 'Plank', icon: Icons.straighten_rounded, defaultReps: 60),
  QuickChipExercise(
      name: 'Burpees',
...
class QuickChipSettingsNotifier extends _$QuickChipSettingsNotifier {
  @override
  ...
  Future<QuickChipSettings> _loadSettings() async {
    try {
      ...
      final record =
          await DatabaseService.settingsStore.record('quick_chips').get(db);
      ...
      final chips =
          (record['enabled'] as List?)?.cast<String>() ?? defaultEnabledChips;
  ...
  Future<void> toggleChip(String chipName) async {
    final current = state.value ??
        const QuickChipSettings(enabledChips: defaultEnabledChips);
  ...
  Future<void> resetToDefaults() async {
    ...
    state = const AsyncValue.data(
        QuickChipSettings(enabledChips: defaultEnabledChips));
  }
}
...
class WorkoutSettings {
  ...
  WorkoutSettings copyWith({bool? autoStartTimerOnFirstRep}) {
    return WorkoutSettings(
      autoStartTimerOnFirstRep:
          autoStartTimerOnFirstRep ?? this.autoStartTimerOnFirstRep,
    );
  }
}
...
class WorkoutSettingsNotifier extends _$WorkoutSettingsNotifier {
  ...
  Future<WorkoutSettings> _loadSettings() async {
    try {
      ...
      return WorkoutSettings(
        autoStartTimerOnFirstRep:
            record['autoStartTimerOnFirstRep'] as bool? ?? true,
      );
    } catch (e) {
      return const WorkoutSettings();
    }
